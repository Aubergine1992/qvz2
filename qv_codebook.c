 ...
 * store them inside the cluster data structure
 */
void generate_codebooks_distortion(struct qv_block_t *info) {
    // Stuff for state allocation and mixing
    double ratio;
    ...
    // Handle column zero specially
    rate = get_entropy(get_cond_pmf(in_pmfs, 0, 0));
    ratio = optimize_for_distortion(get_cond_pmf(in_pmfs, 0, 0), dist, opts->distortion, &q_lo, &q_hi, opts->verbose);
    q_lo->ratio = ratio;
    q_hi->ratio = 1-ratio;
    //total_mse = ratio*q_lo->mse + (1-ratio)*q_hi->mse;
    store_cond_quantizers(q_lo, q_hi, ratio, q_list, 0, 0);
    ...
    for (column = 1; column < info->columns; column++) {
        ...
        for (j = 0; j < q_output_union->size; ++j) {
            // Find and save quantizers
            ratio = optimize_for_distortion(xpmf_list->pmfs[j], dist, opts->distortion, &q_lo, &q_hi, opts->verbose);
            q_lo->ratio = ratio;
            q_hi->ratio = 1-ratio;
            store_cond_quantizers_indexed(q_lo, q_hi, ratio, q_list, column, j);
